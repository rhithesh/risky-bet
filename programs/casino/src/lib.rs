use anchor_lang::{prelude::*,
  ...
  pub mod casino {
    ...
    pub fn close_pdas(ctx:Context<VaultC>)->Result<()>{
  
      // anchor_lang::prelude::close(ctx.accounts.vault_account.to_account_info(), ctx.accounts.user.to_account_info())?;
      msg!("Sucesssfully closed the pdas");
      Ok(())
  ...
  #[derive(Accounts)]
  pub struct VaultC<'info>{
      #[account(mut)]
      pub user: Signer<'info>, // The user who owns the PDA
